    ...
    <div class="flex h-full flex-col divide-y divide-gray-200 dark:divide-slate-800">
      <header class="sticky top-0 z-10 flex flex-col gap-3 border-b border-slate-200 bg-slate-50/92 px-3 py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-950/88 md:static md:border-b-0 md:bg-transparent md:backdrop-blur-0 md:dark:bg-transparent">
        ...
        <div class="flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
          <span class="rounded-full bg-slate-100 px-3 py-1 dark:bg-slate-900 dark:text-slate-300">
            {{ selectedAccount?.nickname ? `当前账号 ${selectedAccount.nickname}` : '请选择一个公众号' }}
          </span>
          <span class="rounded-full bg-blue-50 px-3 py-1 text-blue-600 dark:bg-blue-500/15 dark:text-blue-300">文章 {{ globalRowData.length }} 篇</span>
          <span class="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">已选择 {{ selectedCount }} 篇</span>
        </div>
      </header>
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div class="min-w-0 flex-1 space-y-3">
                    <div class="flex items-start gap-3">
                      ...
                      <div
                        v-else
                        class="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-slate-100 text-slate-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500"
                      >
                      ...
                      <div class="min-w-0 flex-1 space-y-2">
                        <div class="flex flex-wrap items-center gap-2 text-[11px]">
                          ...
                          <span
                            ...
                            class="rounded-full bg-rose-100 px-2.5 py-1 font-medium text-rose-700 dark:bg-rose-500/15 dark:text-rose-300"
                          >
                            已删除
                          </span>
                        </div>
                    ...
                    <div class="grid grid-cols-4 gap-2 rounded-2xl bg-slate-50 px-3 py-2 text-center text-[11px] text-slate-500 dark:bg-slate-800/70 dark:text-slate-300">
                      <div>
                        <p class="font-medium text-slate-700 dark:text-slate-100">{{ article.readNum || 0 }}</p>
                        <p>阅读</p>
                      </div>
                      <div>
                        <p class="font-medium text-slate-700 dark:text-slate-100">{{ article.oldLikeNum || 0 }}</p>
                        <p>点赞</p>
                      </div>
                      <div>
                        <p class="font-medium text-slate-700 dark:text-slate-100">{{ article.shareNum || 0 }}</p>
                        <p>分享</p>
                      </div>
                      <div>
                        <p class="font-medium text-slate-700 dark:text-slate-100">{{ article.commentNum || 0 }}</p>
                        <p>评论</p>
                      </div>
                    </div>
