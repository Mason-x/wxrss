...
export const DEFAULT_AI_SUMMARY_SYSTEM_PROMPT = `你是一名高信噪比的信息摘要编辑，擅长从公众号文章、博客、网文、评论文、教程文、资讯文、观点文中，快速提炼“这篇文章到底讲了什么”以及“值不值得读原文”。
...
function looksLikeMojibake(text: string): boolean {
  return /[婵炶揪缍佸浼存偣閸ヮ剚鏅梺闈涙閸嬫捇顢氶悶鐐存媴閻樿櫕笑閻犲洨鏌夌欢顓㈠礄濞达綁寮伴悹鍥晬闁靛棌鍋撻鐞瀅]/.test(text);
}
...
function shouldReplaceWithDefaultAiTagDefinitions(list: AiTagDefinition[]): boolean {
  ...
  const variables = new Set(
    list.map(item => normalizeAiTagVariable(item.variable, item.label)).filter(Boolean)
  );

  return variables.size === LEGACY_DEFAULT_TAG_VARIABLES.size
...
function isLegacyDefaultAiSummaryPrompt(text: string): boolean {
  return text.includes('"rating"')
    ...
    || (text.includes('一句话总述') && text.includes('核心要点') && !text.includes('"tags"'));
}

function isLegacyDefaultAiTagPrompt(text: string): boolean {
  return text.includes('{{low_signal}}') || text.includes('只能返回一个');
}

function isLegacyDefaultAiDailyReportPrompt(text: string): boolean {
  return text.includes('{{low_signal}}') || text.includes('rating');
}
